...
int shell2(int c, char **v)
{
        ...
        while (1)
        {
                ...
                if (_strlen(input) == 0 || _strlen(input) == 1)
                        continue;
                semi_column_str = _strtok(input, ";");

                if (semi_column_str)
                {
                        while (semi_column_str != NULL)
                        {
                                pid_t pids = fork();
                                printf("%s", semi_column_str);
                                tokens = input_to_array(semi_column_str);
                                run_command(c, v, pids, tokens);
                                semi_column_str = _strtok(NULL, ";");
                        }
                }
                else
                {
